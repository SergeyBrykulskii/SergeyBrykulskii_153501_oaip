...
private:
	...
	struct Node
	{
		...
		Node(Key key) : data(key), left(nullptr), right(nullptr), height(1), parent(nullptr) {}
	};
...
public:
	...
	Node* insert(Node* p, Key key) // вставка ключа k в дерево с корнем p
	{
	...
	struct ListIterator {
		typedef typename std::list<Key>::iterator iterator;
		iterator it;
	...
	struct constListIterator {
		typedef typename std::list<Key>::const_iterator iterator;
		iterator it;
